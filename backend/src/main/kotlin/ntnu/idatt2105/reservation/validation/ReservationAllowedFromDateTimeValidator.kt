...
class ReservationAllowedFromDateTimeValidator :
	...
	override fun initialize(
	    constraintAnnotation: ReservationAllowedFromDateTime?
	) {
	}
	...
	override fun isValid(
		...
		val isStartingAfterEarliestTimeOfDay = reservation.fromTime?.isAfter(
			reservation.fromTime.with(ReservationConstants.EARLIEST_RESERVATION_TIME_OF_DAY)
		) ?: false
		val isStartingBeforeLatestTimeOfDay = reservation.fromTime?.isBefore(
			reservation.fromTime.with(ReservationConstants.LATEST_RESERVATION_TIME_OF_DAY)
		) ?: false
